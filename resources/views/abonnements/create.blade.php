@extends('layouts.app')
@section('title','Nouvel abonnement — SikaFlow')
@section('content')
<section class="max-w-3xl mx-auto px-4 py-8 sm:py-12">
  <div class="mb-8 reveal">
    ...
    <h1 class="text-2xl sm:text-3xl font-bold text-night-900 tracking-tight">Enregistrer un abonnement</h1>
    <p class="text-slate-500 text-sm mt-1">Canal+, Internet, streaming — suivez vos prélèvements récurrents.</p>
  </div>

  <form method="POST" action="{{ route('abonnements.store') }}" class="card p-6 sm:p-8 space-y-5 reveal reveal-delay-1">
    @csrf
    <div class="grid md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input type="date" name="prochaine_echeance" value="{{ old('prochaine_echeance', isset($abonnement) ? $abonnement->prochaine_echeance->format('Y-m-d') : now()->format('Y-m-d')) }}" class="input" required>
      </div>
    ...
    <button type="submit" class="btn-primary w-full !py-4 relative overflow-hidden">
      <i class="fa-solid fa-circle-check"></i> Enregistrer l'abonnement
