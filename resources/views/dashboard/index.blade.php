...
<section class="max-w-6xl mx-auto px-4 py-10">
  <div class="flex justify-between items-end mb-8">
    <div>
      <h1 class="text-3xl font-bold text-night-900">Bonjour, {{ auth()->user()->nom }} 👋</h1>
      <p class="text-slate-500">Voici l'état de vos engagements récurrents.</p>
    </div>
    <a href="{{ route('prets.create') }}" class="px-4 py-2.5 rounded-xl bg-sika-600 text-white font-semibold hover:bg-sika-700 shadow-soft">+ Nouveau prêt</a>
  </div>
  ...
  <div class="grid md:grid-cols-3 gap-6 mt-8">
    <div class="md:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-soft">
      <div class="p-5 border-b border-slate-100 flex justify-between items-center">
        ...
        <a href="{{ route('prets.index') }}" class="text-sm text-sika-700">Voir tous les prêts →</a>
      </div>
      <div class="divide-y divide-slate-100">
        @forelse($prochainesEcheances as $e)
          <div class="p-4 flex justify-between items-center">
            <div>
              <div class="font-medium">{{ $e->pret->creancier->nom }} · #{{ $e->numero }}</div>
              <div class="text-sm text-slate-500">Échéance le {{ $e->date_echeance->format('d/m/Y') }}</div>
            </div>
            <div class="text-right">
              <div class="font-semibold">{{ number_format($e->montant,0,',',' ') }} FCFA</div>
        ...
        @empty
          <div class="p-8 text-center text-slate-500">Aucune échéance à venir — bravo ! 🎉</div>
        @endforelse
      </div>
    </div>
